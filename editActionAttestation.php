...
if (isset($_GET['Reference_Reçu']) AND !empty($_GET['Reference_Reçu'])) {
    $getReferenceRecu = $_GET['Reference_Reçu'];
    // Récupération des informations de l'étudiant

    $recuppaiement= $bdd->prepare('SELECT * FROM paiement WHERE Reference_Reçu = ?');
    $recuppaiement->execute(array( $getReferenceRecu)); 

    // Vérification de l'existence de l'étudiant 
    if ($recuppaiement ->rowCount() >0) {
        $recuppaiementIfExists=$recuppaiement->fetch();
        $ReferenceRecu =   $recuppaiementIfExists['Reference_Reçu'];
        $DateRecu =  $recuppaiementIfExists['Date_Reçu'];
        $DateremiseRecu =  $recuppaiementIfExists['Date_remise_Reçu'];
       
     
     if (isset($_POST['update'])) {
       // Les données de l'étudiant 
       $newReferenceRecu = htmlspecialchars($_POST['ReferenceRecu']);
       $date1=strtr($_REQUEST['DateRecu'], '/', '-');
       $newDaterecu= date('Y-m-d', strtotime($date1));
       $date2=strtr($_REQUEST['DateremiseRecu'], '/', '-');
       $newDateremiseRecu= date('Y-m-d', strtotime($date2));
  

       // Script de modification
       $updatepaiement = $bdd->prepare('UPDATE paiement SET Reference_Reçu=?, Date_Reçu=?, Date_remise_Reçu=? WHERE Reference_Reçu=?');
       $updatepaiement->execute(array($newReferenceRecu,$newDaterecu,$newDateremiseRecu,$getReferenceRecu));

       header("Location:liste_AttestationInscription.php");
     }
        
      }else {
        $errorMsg = "Aucune attestation trouvée" ;
    }
...
<form action="" method="POST">
      ...
      <div class="main-content">
        <section class="section">
          <div class="section-header">
            <h1>Formulaire d'enregistrement d'attestation d'inscription</h1>
          ...
          <div class="section-body">
            <h2 class="section-title">Ajout d'attestation</h2>
            ...
            <div class="card-body">
                    <div class="alert alert-info">
                      <b>
                      <a href='liste_AttestationInscription.php'>Consulter la liste des attestations delivrées</a>
                      </b>
        ...
        ?>
                    ...
                    <div class="form-group">
                      <label>Reference du Reçu</label>
                      <input type="varchar" class="form-control" name="ReferenceRecu" value="<?=$ReferenceRecu ; ?>">
                    </div>

                   
                    <div class="form-group">
                      <label>Date du reçu</label>
                      <input type="date" class="form-control" name="DateRecu" value="<?=$DateRecu; ?>">
                    </div> 
                   
                    <div class="form-group">
                      <label>Date de remise </label>
                      <input type="date" class="form-control" name="DateremiseRecu" value="<?=$DateremiseRecu; ?>">
                    </div>
      ...
      <footer class="main-footer">
        <div class="footer-left">
          Copyright &copy; 2022 <div class="bullet"></div>  By <a href="https://nauval.in/">Ousseynou Dione</a>
        </div>
